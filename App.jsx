...
// Reference colors for the 4 mask classes (Other / Non-invasive / Invasive /
// Necrosis). The mask PNG's actual pixel colors are baked in by the model
// pipeline, so these are editable in the UI in case the real palette differs.
const SEG_CLASSES = [
  { id: 1, label: "Other",        color: "#8a94ad" },
  { id: 2, label: "Non-invasive", color: "#3c8cdc" },
  { id: 3, label: "Invasive",     color: "#e84c6a" },
  { id: 4, label: "Necrosis",     color: "#2b2f38" },
];
...
// ─── Mini chart (no external deps — plain SVG) ────────────────────────────────
...
// ─── 1. Home & 2. Gallery Screens ─────────────────────────────────────────────
function HomeScreen({ onAnalyze, onOpenGallery }) {
  return (
    <div style={{background:T.bg1,minHeight:"100vh",color:T.txt,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",fontFamily:"system-ui"}}>
      <div style={{textAlign:"center",background:T.bg2,padding:"40px 60px",borderRadius:12,border:`1px solid ${T.bdr}`,boxShadow:"0 10px 30px rgba(0,0,0,0.5)",maxWidth:600,width:"100%"}}>
        <h2 style={{margin:0,fontSize:22,marginBottom:5}}>MammoMap Case Analysis</h2>
        <p style={{color:T.txt2,fontSize:13,marginBottom:24}}>Upload patient H&E ROIs or browse cached cases.</p>
        <button onClick={onOpenGallery} style={{background:T.bg3, border:`1px solid ${T.bdr}`, padding:"10px 20px", borderRadius:6, color:T.txt, cursor:"pointer", transition:"border .2s", fontWeight:600}}>
          Enter Demo Gallery (Cached)
        </button>
      </div>
    </div>
  );
...
function DemoGallery({ onBack, onSelectPatient }) {
  return (
    <div style={{background:T.bg1,minHeight:"100vh",color:T.txt,fontFamily:"system-ui",padding:40}}>
      <div style={{maxWidth: 1000, margin: "0 auto"}}>
        <button onClick={onBack} style={{background:"transparent",border:"none",color:T.txt2,cursor:"pointer",fontSize:13,marginBottom:20}}>← Back to Home</button>
        <h2 style={{margin:0,fontSize:24,marginBottom:30}}>Cached Demo Cohort</h2>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill, minmax(300px, 1fr))",gap:20}}>
          {DEMO_PATIENTS.map(p => (
            <div key={p.id} onClick={() => onSelectPatient(p.rois)} style={{background:T.bg2, border:`1px solid ${T.bdr}`, borderRadius:10, padding:20, cursor:"pointer"}}>
              ...
              <div style={{fontSize:12, color:T.txt2, marginBottom:15}}>{p.subtype} • {p.rois.length} ROIs</div>
              <div style={{display:"flex", gap:10, overflowX:"auto"}}>
                {p.rois.map((r, i) => (
                  <img key={i} src={r.src} alt={r.label} style={{width:64, height:64, objectFit:"cover", borderRadius:6}} />
                ))}
              </div>
            </div>
          ))}
...
function AnalysisViewer({ rois, onBack }) {
  ...
  useEffect(() => {
    ...
    const img = new Image();
    img.onload = () => { ihcImgCache.current[cacheKey] = img; setIhcImage(img); setIhcImageStatus("loaded"); };
  ...
  const draw = useCallback(() => {
    ...
    ctx.restore();
  }, [groups, images, activeIdx, spatialData, bounds, geneLayers, ihcImage]);
            ...
            <>
              ...
              {groups.find(g=>g.id==="ihc").layers[0].vis && (
                <div style={{position:"absolute", top:12, left:12, background:"rgba(13,16,24,0.88)", border:`1px solid ${T.bdr}`, borderRadius:8, padding:"6px 10px", fontSize:11, color:T.purple, fontWeight:600}}>
                  ...
                  {activeRoi?.ihc?.[activeIHCMarker]?.status ? ` • ${activeRoi.ihc[activeIHCMarker].status}` : ""}
                ...
                <div key={g.id} style={{background:T.bg3,borderRadius:6,padding:"8px"}}>
                  ...
                  {g.id === "ihc" && (
                    <div style={{marginTop:10, paddingTop:10, borderTop:`1px solid ${T.bdr}`}}>
                      ...
                      {activeRoi?.ihc?.[activeIHCMarker] ? (
                        <div style={{background:T.bg1, border:`1px solid ${T.bdr}`, borderRadius:6, padding:8, fontSize:11}}>
                          {Object.entries(activeRoi.ihc[activeIHCMarker]).filter(([k])=>!["src","base64"].includes(k)).map(([k,v])=>(
...
export default function App() {
  ...
  if (view === "gallery") return <DemoGallery onBack={() => setView("home")} onSelectPatient={(rois) => { setCaseRois(rois); setView("viewer"); }} />;
  return <HomeScreen onAnalyze={(rois) => { setCaseRois(rois); setView("viewer"); }} onOpenGallery={() => setView("gallery")} />;
}
